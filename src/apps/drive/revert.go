...
var revert = &cobra.Command{
	Use:   "revert [file name] [version]",
	Short: "Reverts a file to a specific previous version",
	...
	RunE: func(cmd *cobra.Command, args []string) (err error) {
		...
		if res.StatusCode != http.StatusOK {
			...
			return fmt.Errorf("revert/copy failed (Status %d): %s", res.StatusCode, msg)
		}
		...
		log.Info("Success", "action", fmt.Sprintf("file '%s' reverted to version %s", name, version))
		return
	},
}
