...
var lock = &cobra.Command{
	Use:   "lock [name]",
	Short: "Locks a file on the remote server to prevent changes",
	Args:  cobra.MinimumNArgs(1),
	RunE: func(cmd *cobra.Command, args []string) (err error) {
		name := args[0]

		req, err := http.NewRequest("POST", "http://app.starducc.mathrock.xyz/lock/"+name, nil)
		if err != nil {
		...
		log.Info("Success", "action", fmt.Sprintf("file '%s' successfully locked", name))
		return
	},
}
